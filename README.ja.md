...
同じ「指値・逆指値」でも、**新規** と **既存ポジションの決済** では JSON が違います。ForceOpen（両建て）では、反対方向の standalone Stop/Limit が **決済ではなく新規ショート／ロング** になることがあります。

| 意図 | 推奨 API |
|------|----------|
| 新規（成行／指値／逆指値） | `client.open_market` / `open_limit` / `open_stop`（`is_force_open` 必須）、または Layer 2 `PositionOpen` |
| FIFO／ネット相殺決済 | `client.close_fifo_*`、または `PositionClose.fifo_*` |
| ForceOpen 建玉の明示決済 | `client.close_force_open_*`（`iter_open_positions` の `position_id`）、または `PositionClose.force_open_*` |
| FO 残骸の一掃 | `client.flatten_force_open` |

ForceOpen を閉じるのに裸の `MarketOrder` / `LimitOrder` / `StopOrder` を使わないでください。詳細: [docs/contrib/orders.md](docs/contrib/orders.md)、[docs/examples/close_position.md](docs/examples/close_position.md)、[docs/contrib/client.md](docs/contrib/client.md)。
...
- **Layer 3（高レベル API・推奨）**: `SaxoClient`, `OptionTrader`
  - 取引の主入口。`SaxoClient` では意図別の `open_*` / `close_fifo_*` / `close_force_open_*` / `flatten_force_open` / `iter_open_positions` を優先
  - レガシーの `market_order` / `limit_order` / `stop_order` 等は簡易用途向け。ForceOpen 決済には使わない
  - ティッカー（Symbol）を `Uic` に解決（複数ヒット時は `PrimaryListing` フォールバック）
  - `AccountKey` を注入し、ネストした注文パラメータを構築する（旧 `SaxoTrader` は削除済み。import しないこと）
- **Layer 2（意図別／注文ビルダー）**: 推奨は `PositionOpen` / `PositionClose`。低レベルは `MarketOrder` / `LimitOrder` / `StopOrder` など
  - 注文種別だけでなく **新規か決済か・FIFO か ForceOpen か** で選ぶ
  - `MarketCloseOrder` は削除済み。代わりに `PositionClose.force_open_*`

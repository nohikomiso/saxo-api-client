...
class AllocationKeyStatus(_FlexModel):
    Active: Optional[Any] = Field(default=None, alias="Active", description="The key is active and can be used for allocations.")
    DeactivateAfterOrderPlacement: Optional[Any] = Field(default=None, alias="DeactivateAfterOrderPlacement", description="Indicates that the key will be deactivated after order placement.")
    Inactive: Optional[Any] = Field(default=None, alias="Inactive", description="Indicates that key is inactive/deleted and should not be used for any further allocation.")
    OneTime: Optional[Any] = Field(default=None, alias="OneTime", description="Indicates that the key has been created to one-time allocation.")

class GetalistofexistingallocationkeysRequest(_FlexModel):
    ...
    AccountKey: Optional[Any] = Field(default=None, alias="AccountKey", description="Optional account key to filter out allocation keys.")
    ClientKey: Optional[Any] = Field(default=None, alias="ClientKey", description="Client key for which allocation keys are to be returned.")
    IncludeSubClients: Optional[bool] = Field(default=None, alias="IncludeSubClients", description="To indicate whether to include allocation keys for sub clients. Default is true.")
    Statuses: Optional[AllocationKeyStatus] = Field(default=None, alias="Statuses", description="Comma separated list of statuses for keys to include. Default is 'Active'.")

class AllocationUnitType(_FlexModel):
    CashAmount: Optional[Any] = Field(default=None, alias="CashAmount", description="Distribution is done on a cash amount basis. In this case the total sum of CashAmounts in the ParticipatingAccounts array must add up to the amount specified in the order, or at least one of the accounts in the ParticipatingAccounts must have the AcceptRemainderAmount field set to true.")
    Percentage: Optional[Any] = Field(default=None, alias="Percentage", description="Distribution is done on a percentage basis. In this case the total sum of UnitValues in the ParticipatingAccounts array must add up to 100.")
    Unit: Optional[Any] = Field(default=None, alias="Unit", description="Distribution is done using units allocation. In this case the total sum of UnitValues in the ParticipatingAccounts array must add up to the amount specified in the order, or at least one of the accounts in the ParticipatingAccounts must have the AcceptRemainderAmount field set to true.")

class MarginHandlingType(_FlexModel):
    Reduce: Optional[Any] = Field(default=None, alias="Reduce", description="If on margin, reduce the order with the amount the account on margin would have received.")
    Reject: Optional[Any] = Field(default=None, alias="Reject", description="If on margin, reject the entire order rather than doing part allocation.")

class AllocationKeyAccountInfo(_FlexModel):
    AcceptRemainderAmount: Optional[bool] = Field(default=None, alias="AcceptRemainderAmount", description="Specifies if an account can accept the remaining units, if applicable.")
    AccountId: Optional[str] = Field(default=None, alias="AccountId", description="AccountId of account, which is part of allocation. When creating allocation key, then either AccountKey or AccountId is required.")
    AccountKey: Optional[Any] = Field(default=None, alias="AccountKey", description="AccountKey of account, which is part of allocation. When creating allocation key, then either AccountKey or AccountId is required.")
    Priority: Optional[int] = Field(default=None, alias="Priority", description="If several accounts are defined with AcceptRemainderAmount=true, the priority determines the priority by which remainders are distributed between these accounts Also when allocating partial fills, it is allocated in the prioritized order")
    UnitValue: Optional[float] = Field(default=None, alias="UnitValue", description="The actual amount or percentage of the total trade to be distributed to this account, see AllocationUnitType for further explanation.")

class CreateanallocationkeyRequest(_FlexModel):
    AllocationKeyName: Optional[str] = Field(default=None, alias="AllocationKeyName", description="Name of the allocation key.")
    AllocationUnitType: Optional[AllocationUnitType] = Field(default=None, alias="AllocationUnitType", description="Specifies how to interpret the value of UnitValue in the AllocationKeyAccountInfo.")
    MarginHandling: Optional[MarginHandlingType] = Field(default=None, alias="MarginHandling", description="Represents the behavior if because of margin reason entire order can not be processed.")
    OneTime: Optional[bool] = Field(default=None, alias="OneTime", description="Whether the key is for one time use.")
    OwnerAccountKey: Optional[Any] = Field(default=None, alias="OwnerAccountKey", description="Account key for the account to this allocation key belongs to.")
    ParticipatingAccountsInfo: Optional[List[AllocationKeyAccountInfo]] = Field(default=None, alias="ParticipatingAccountsInfo", description="Allocation participants.")

class GetdetailedinformationaboutanallocationkeyRequest(_FlexModel):
    AllocationKeyId: Optional[str] = Field(default=None, alias="AllocationKeyId", description="allocation key ID")

class DeleteallocationkeyRequest(_FlexModel):
    AllocationKeyId: Optional[str] = Field(default=None, alias="AllocationKeyId", description="allocation key ID")
...
class GetdistributedamountforagivenallocationkeyRequest(_FlexModel):
    AllocationKeyId: Optional[str] = Field(default=None, alias="AllocationKeyId", description="Allocation key id")
    AssetType: Optional[AssetType] = Field(default=None, alias="AssetType", description="Asset type. Optional, must be used in conjunction with name='uic'")
    OrderAmountType: Optional[OrderAmountType] = Field(default=None, alias="OrderAmountType", description="Indicates if the total amount is specified as lots/shares/contracts or as a monetary purchase amount.")
    Totalamount: Optional[float] = Field(default=None, alias="Totalamount", description="Total amount to distribute between accounts associated with the allocationKey")
    Uic: Optional[int] = Field(default=None, alias="Uic", description="Unique instrument identification. Optional, must be used in conjunction with 'assetType'")

...
class RequestExpirySpecification(_FlexModel):
    Index: Optional[int] = Field(default=None, alias="Index", description="The expiry index")
    StrikeStartIndex: Optional[int] = Field(default=None, alias="StrikeStartIndex", description="The starting index of the strikes to return The number of strikes counted from this is determined by the MaxNumberOfStrikes of the request")
...
class OptionsChainRequest(_FlexModel):
    ...
    Expiries: Optional[List[RequestExpirySpecification]] = Field(default=None, alias="Expiries", description="Expiries are a collection of expiries requested and the window of strike-rows for each expiry. This allows us to ask explicitly for a certain selection of the total set of option chain data. If not provided the service picks the 3 first expiries and assume a centered start-index for all expiries. Asking for data for an expiry that does not exist or a start index higher than the number of strike rows available for that expiry, is not exactly an error and I believe it should just lead to no data being returned.")
    Identifier: Optional[int] = Field(default=None, alias="Identifier", description="The instrument's identifier. Use UIC for FX. For Contract Options use group root id or option root id - both will return all options within specified group root or in case of option root id - all options that are in the same group as specified option root.")
    MaxStrikesPerExpiry: Optional[int] = Field(default=None, alias="MaxStrikesPerExpiry", description="Signals a request to only get N strike-rows per expiry. Default is 12. If no explicit StartIndex is provided, the N rows are assumed to be centered on the strike closes to the mid-spot price. I.e. it could be for an expiry with 16 strike-rows, a request to show 12 strikes-per-expiry means that the start-index for that expiry will be 2, leading the strike-rows to be returned to be the [2-13] or [2-14[ range.")

class CreateanactiveoptionschainsubscriptionRequest(_FlexModel):
    Arguments: Optional[OptionsChainRequest] = Field(default=None, alias="Arguments", description="Arguments for the subscription request.")
...
class ModifyanexistingoptionschainsubscriptionThisreplacestheobsoleteversionsubscriptionsactivecontextIdreferenceIdRequest(_FlexModel):
    ContextId: Optional[str] = Field(default=None, alias="ContextId", description="")
    Expiries: Optional[List[RequestExpirySpecification]] = Field(default=None, alias="Expiries", description="Expiries are a collection of expiries requested and the window of strike-rows for each expiry. This allows us to ask explicitly for a certain selection of the total set of option chain data. If not provided the service picks the 3 first expiries and assume a centered start-index for all expiries. Asking for data for an expiry that does not exist or a start index higher than the number of strike rows available for that expiry, is not exactly an error and I believe it should just lead to no data being returned.")
    MaxStrikesPerExpiry: Optional[int] = Field(default=None, alias="MaxStrikesPerExpiry", description="Signals a request to only get N strike-rows per expiry. Default is 12. If no explicit StartIndex is provided, the N rows are assumed to be centered on the strike closes to the mid-spot price. I.e. it could be for an expiry with 16 strike-rows, a request to show 12 strikes-per-expiry means that the start-index for that expiry will be 2, leading the strike-rows to be returned to be the [2-13] or [2-14[ range.")
    ReferenceId: Optional[str] = Field(default=None, alias="ReferenceId", description="")

class RemoveanoptionschainsubscriptionRequest(_FlexModel):
...
class ResetanoptionschainsubscriptionAtTheMoneyRequest(_FlexModel):
    ContextId: Optional[str] = Field(default=None, alias="ContextId", description="The context id part of the streaming session (used to identify the subscription within a streaming session).")
    ReferenceId: Optional[str] = Field(default=None, alias="ReferenceId", description="Unique id of the subscription")
